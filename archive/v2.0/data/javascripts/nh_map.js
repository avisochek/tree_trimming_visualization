	...
	function plot_points(data){
		...
		data.forEach(function(d){
			...
			if (date_closed & date_acknowledged ) {
				time_till_closed = Math.round((date_closed.valueOf() - date_created.valueOf())/86400000)
			}
...
//debugger;
		...
		function get_color(d){
			if (d.date_acknowledged & d.date_closed){return "green";}
			else{return "red";}
		};
		...
		var get_x_2015 = d3.time.scale().domain([new Date('2015-01-01'),new Date('2015-07-01')]).range([0, 600])
	...
	d3.csv("data/nhv5.1.csv", function(d) {
		return d;
  }, plot_points);
};
